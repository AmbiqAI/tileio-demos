...
#define MAX_RR_PEAKS (4*PK_DATA_LEN/SAMPLE_RATE)
#define VERBOSE 1
...
static float32_t pkArena[5*PK_DATA_LEN];

static float32_t spo2Coefs[3] = {1.5958422, -34.6596622, 112.6898759};

static float32_t ppg1Clean[PK_DATA_LEN];
static float32_t ppg2Clean[PK_DATA_LEN];
static float32_t ecgClean[PK_DATA_LEN];
static float32_t qrsClean[PK_DATA_LEN];
...
static uint8_t ppg1Mask[PK_DATA_LEN];
static uint8_t ppg2Mask[PK_DATA_LEN];
static uint8_t ecgMask[PK_DATA_LEN];
...
uint32_t
sk_metrics(float32_t *ppg1Data, float32_t *ppg2Data, float32_t *ecgData, size_t dataLen) {
    /**
    ...
    spo2 = pk_compute_spo2_in_time(ppg1Clean, ppg2Clean, ppg1Mean, ppg2Mean, PK_DATA_LEN, spo2Coefs, SAMPLE_RATE);
